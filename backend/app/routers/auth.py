...
async def create_user(user_email, user_full_name, user_name, balance=1.00):  ## in $$dolars by default balance is float
    ...
    try:
        ...
        bearer_token = str(uuid.uuid4())  # Generate a new Bearer token (for authentication)
        ...
        async with conn.cursor() as cursor:
            ...
            """
            values = (user_id, user_name, api_token, bearer_token, user_fname, user_lname, user_email, balance)
        ...
        logger.exception("Error occurred while creating user with email: %s. Error: %s", str(e), user_email)
        raise HTTPException(status_code=500, detail=f"An error occurred while creating user: {str(e)}") from e
    ...
    try:
        ...
        async with conn.cursor() as cursor:
                ...
                if result:
                    ...
                    return "No API token or balance found for the user", None  # If no result, return None (user not found)
        ...
        async with conn.cursor() as cursor:
                ...
                WHERE user_id = %s
                """
                await cursor.execute(query, (new_api_token, user_id))  # Execute the update query with parameters
                
                await conn.commit()  # Commit the transaction to make the change persistent
        ...
        await redis.hset(f"llm_api_token:{new_api_token}", 
                          mapping = {
                                "user_id": user_id,
                                "balance": str(balance),
                            }
...
async def github_auth(request_data: GitHubAuthRequest, redis: Redis = Depends(get_redis_client),
    ...
    try:
        ...
        if not redis.setnx(f"oauth_code:{github_code}", "used"):
            ...
            raise HTTPException(status_code=400, detail="This OAuth code has already been used.")
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                logger.error(f"Failed to fetch GitHub token. Status code: {response.status_code}, Response: {response.text}")
            ...
            if not access_token:
                logger.error(f"Invalid GitHub token response: No access token found :{github_data}")
            ...
            if user_info_response.status_code != 200:
                logger.error(f"Failed to fetch GitHub user info. Status code: {user_info_response.status_code}, Response: {user_info_response.text}")
                raise HTTPException(status_code=400, detail="Failed to fetch GitHub user info")
            
            if user_email_response.status_code != 200:
                logger.error(f"Failed to fetch GitHub user email. Status code: {user_email_response.status_code}, Response: {user_info_response.text}")
        ...
        user_email = user_email_info[0].get("email") if isinstance(user_email_info, list) else None
        ...
        if not user_full_name:
            logger.warning("GitHub user full name not found in the response, using a default name.")
        ...
        user_id, api_token, bearer_token = await create_user(user_email, user_full_name, user_name)
        return {"status": "success", "user_id": user_id, "api_token": api_token, "bearer_token": bearer_token}

    except Exception as e:
        logger.exception(f"Error occurred during GitHub authentication: {str(e)}")
...
async def regenerate_token(credentials: HTTPAuthorizationCredentials = Depends(security)):
    try:
        ...
        logger.exception(f"Redis error occurred while regenerating API keys: {str(redis_err)}")
        raise HTTPException(status_code=500, detail="A Redis error occurred while processing your request.")
    
    except Exception as e:
        logger.exception(f"Error occurred while regenerating API keys: {str(e)}")
        raise HTTPException(status_code=500, detail=f"An error occurred: {str(e)}")